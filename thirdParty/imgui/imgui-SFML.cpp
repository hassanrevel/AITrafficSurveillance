...
[[nodiscard]] ImVec2 toImVec2(const sf::Vector2f &v) { return {v.x, v.y}; }

[[nodiscard]] sf::Vector2f toSfVector2f(const ImVec2 &v) { return {v.x, v.y}; }
...
[[nodiscard]] GLuint
convertImTextureIDToGLTextureHandle(ImTextureID textureID) {
...
[[nodiscard]] SpriteTextureData getSpriteTextureData(const sf::Sprite &sprite) {
  ...
  return {toImVec2(textureRect.position.componentWiseDiv(textureSize)),
          toImVec2((textureRect.position + textureRect.size)
                       .componentWiseDiv(textureSize)),
...
struct WindowContext {
  ...
  std::optional<sf::Texture>
      fontTexture; // internal font atlas which is used if user doesn't set
                   // a custom sf::Texture.
...
void SetCurrentWindow(const sf::Window &window) {
  auto found = std::find_if(s_windowContexts.begin(), s_windowContexts.end(),
                            [&](std::unique_ptr<WindowContext> &ctx) {
                              return ctx->window->getNativeHandle() ==
...
bool UpdateFontTexture() {
  ...
  ImTextureID texID =
      convertGLTextureHandleToImTextureID(newTexture.getNativeHandle());
...
void Image(const sf::Texture &texture, const sf::Vector2f &size,
  ...
  ImGui::Image(textureID, toImVec2(size), ImVec2(0, 0), ImVec2(1, 1),
...
void Image(const sf::RenderTexture &texture, const sf::Vector2f &size,
  ...
  ImGui::Image(textureID, toImVec2(size), ImVec2(0, 1),
...
bool ImageButton(const char *id, const sf::Texture &texture,
  ...
  return ImGui::ImageButton(id, textureID, toImVec2(size), ImVec2(0, 0),
...
bool ImageButton(const char *id, const sf::RenderTexture &texture,
  ...
  return ImGui::ImageButton(
      id, textureID, toImVec2(size), ImVec2(0, 1),
...
bool ImageButton(const char *id, const sf::Sprite &sprite,
                 ...
                 const sf::Color &tintColor) {
  auto [uv0, uv1, textureID] = getSpriteTextureData(sprite);
  return ImGui::ImageButton(id, textureID, toImVec2(size), uv0, uv1,
                            toImColor(bgColor), toImColor(tintColor));
}
...
void RenderDrawLists(ImDrawData *draw_data) {
  ...
  assert(io.Fonts->TexRef._TexData !=
         nullptr); // You forgot to create and set font texture
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback) {
        ...
        if (clip_rect.x < static_cast<float>(fb_width) &&
            ...
            clip_rect.z - clip_rect.x >= 0.0f &&
            clip_rect.w - clip_rect.y >= 0.0f) {
